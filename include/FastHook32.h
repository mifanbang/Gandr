...
// FastHook doesn't support amd64
#if !defined(_WIN64)
...
// ---------------------------------------------------------------------------
// class FastHook32 - simple inline hook for hotpatchable stdcall functions
// ---------------------------------------------------------------------------
...
public:
	...
	template <typename F>
	FastHook32(const F* oriFunc, const F* hookFunc) noexcept
...
#define CallTram32(func)		( *(reinterpret_cast<__HELPER_TYPE__(func)*>(reinterpret_cast<uint8_t*>(func) + sizeof(gan::Prolog32)) ) .* __HELPER_TYPE__(func)::ConvertPtr( ::gan::SupportedProlog32::GetTrampoline(::gan::PrologTable32::Query(func)) ) )
